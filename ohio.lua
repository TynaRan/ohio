...
local InfoTab = Window:AddTab('Account Info')
local StatusBox = InfoTab:AddLeftGroupbox('Account Status')

StatusBox:AddLabel("ACCOUNT STATUS")
local StatusLabel = StatusBox:AddLabel("Checking...", false, "AccountStatus")
local DetailsLabel = StatusBox:AddLabel("", true, "AccountDetails")

local function checkAccountStatus()
    local now = os.time()
    local isBanned = false
    local banReason = "None"
    local banCount = 0
    local banEnd = 0
    local lastBanDate = 0
    ...
    for _, entry in ipairs(getgc(true)) do
        if type(entry) == "table" then
            local exes = rawget(entry, "shadowbannedexes") or rawget(entry, "bannedUntil")
            if exes then
                local endTime = type(exes) == "number" and exes or exes.Value
                banCount = banCount + 1
                lastBanDate = math.max(lastBanDate, rawget(entry, "shadowbannedAt") or 0)
                
                if now < endTime then
                    isBanned = true
                    banEnd = endTime
                    banReason = rawget(entry, "shadowbanned") or "Unknown"
                end
            end
        end
    end
    ...
    if isBanned then
        local remaining = banEnd - now
        local days = math.floor(remaining/86400)
        local hours = math.floor((remaining%86400)/3600)
        local mins = math.floor((remaining%3600)/60)
        
        StatusLabel:SetText("🔴 RESTRICTED")
        DetailsLabel:SetText(
            "Reason: "..banReason.."\n"..
            "Ends in: "..days.."d "..hours.."h "..mins.."m\n"..
            "Total restrictions: "..banCount
        )
    else
        StatusLabel:SetText(banCount > 0 and "⚠️ PREVIOUSLY RESTRICTED" or "✅ CLEAN")
        DetailsLabel:SetText(
            "Last checked: "..os.date("%X".."\n"..
            "Total restrictions: "..banCount.."\n"..
            (lastBanDate > 0 and "Last restriction: "..os.date("%x", lastBanDate) or "")
        )
    end
end
